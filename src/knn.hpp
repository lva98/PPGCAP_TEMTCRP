...
namespace knn {
  ...
  struct output {
    ...
    output (std::vector<bool>& r, std::vector<int>& a, std::map<int, int>& mbl, std::map<int, int>& sbl, std::map<int, std::vector<int>> sd, std::map<int, std::vector<int>> md, int m, int s)
      : result(r), attempts(a), mistakes_by_labels(mbl), successes_by_labels(sbl), mistakes_data(md), successes_data(sd), mistakes(m), successes(s) {}
  };
