...
int main (int argc, char *argv[]) {
    ...
    if (argc != 4 && argc != 5) {
        std::cout << "Use: ./method_2 z1.csv z2.csv z3.csv" << std::endl;
        return 1;
    }
    ...
    while (total--) {
        ...
        for (int t = 0; t < t_count; t++) {
            ...
            for (int k = 3, i = 0; k <= 13; k += 1, ++i) {
                ...
                if (output.successes > local_successes) {
                    best_output = cv::makePtr<knn::output>(output);
                    t_best = t;
                    best_k = k;
                    successes = output.successes;
                    local_successes = output.successes;
            ...
            printf("best_local_k %d\n", best_k);
            printf("-\n");

            for (const auto pair: best_output->mistakes_data) {
        ...
        printf("result\n");
        printf("best_t,best_k,successes,mistakes,success_ratio\n");
        printf("%d,%d,%d,%d,%.2lf%\n\n", t_best, best_k, output.successes, output.mistakes, success_ratio);
